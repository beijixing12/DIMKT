...
def _count_dataframe_entities(df: pd.DataFrame, *, question_column: str) -> Dict[str, int]:
    return {
        "users": int(df["user_id"].nunique()) if "user_id" in df.columns else 0,
        "problems": int(df[question_column].nunique()) if question_column in df.columns else 0,
        "interactions": int(df.shape[0]),
    }


def _count_example_entities(examples: Sequence[SequenceExample]) -> Dict[str, int]:
    problems: set[int] = set()
    total_interactions = 0
    for example in examples:
        problems.update(example.question_seq)
        total_interactions += example.seq_len
    return {
        "users": len(examples),
...
def main() -> None:
    ...
    raw_counts = _count_dataframe_entities(raw_df, question_column=args.question_column)
    print(
        "Raw dataset - Users: {users}, Problems: {problems}, Interactions: {interactions}".format(
            **raw_counts
        )
    )
    ...
    cleaned_counts = _count_dataframe_entities(cleaned_df, question_column=args.question_column)
    print(
        "Cleaned dataset - Users: {users}, Problems: {problems}, Interactions: {interactions}".format(
            **cleaned_counts
        )
    )
    ...
    processed_counts = _count_example_entities(examples)
    print(
        "Processed sequences - Users: {users}, Problems: {problems}, Interactions: {interactions}".format(
            **processed_counts
        )
    )
